<!DOCTYPE html>
<html>
<head>
	<title>Uiiverse - Android</title>
	<meta charset="utf-8">
	<link rel="stylesheet" type="text/css" href="https://stackpath.bootstrapcdn.com/bootstrap/4.2.1/css/bootstrap.min.css">
	<link rel="stylesheet" type="text/css" href="css/main.css">
	<link rel="icon" type="image/png" href="data:image/png;base64,">
</head>
<body>
  <div style='font-family:nintendou;'>
    <a href="../">← Go back to Uiiverse.</a>
    <center>
      <br>
      <br>
      <br>
      <br>
      <br>
      <br>
      <br>
      <br>
      <img src="img/uiiverseandandroid.png" width=30% height=30%>
      <b><h1><u>[Uiiverse for Android</u></h1></b> 
      <p>Use Uiiverse on your phone. Just a few taps away.</p>
      <nav>
      <a href="https://downloads.uiiverse.xyz/android/latest/Uiiverse.apk">Download from server.</a>
      <a href="https://uiiverse.miro.gg/android" target="_blank" title="Download from mirror">Download from mirror.</a>	
      </nav>
      <br>
      <small>Before downloading make sure you have unknown sources enabled.</small>
      <br>
      <small>You can enable them by going on Settings > Security > Unknown Sources.</small>
      <br>
      <small>If you have Android 9.0 or above, you can skip this step.</small>
      <br>
      <br>
      <br>
      <br>
      <h3><u>Changelog</u></h3>
      <br>
      <h4>1.4.2 (Released on June 25th, 2019)</h4>
      <p>-Fixed Settings FAB not hiding when toggling fullscreen.</p>
      <p>-Changed FAB icons.</p>
      <br>
      <h4>1.4.1 (Released on June 22nd, 2019)</h4>
      <p>-Bug fixes</p>
      <p>-Changed accent color</p>
      <br>
      <h4>1.4 (Released on June 22nd, 2019)</h4>
      <p>-Added a configuration menu</p>
      <p>-Added an option to disable the startup sound</p>
      <p>-Added multiple languages</p>
      <br>
      <h4>1.3 (Released on February 28th, 2019)</h4>
      <p>-Added a new FAB</p>
      <p>-Animations improved</p>
      <br>
      <h4>1.2 (Released on February 16th, 2019)</h4>
      <p>-Changed the home URL from uiiverse.ml to uiiverse.xyz</p>
      <p>-Added about info to the main screen</p>
      <br>
      <h4>1.1 (Released on February 11th, 2019)</h4>
      <p>-Initial release</p>
      <br>
      <br>
      <br>
      <br>
      <br>
      <br>
    </center>
  </div>
</body>
</html>

     